...
use crate::gpu::registers::{
    ColorPalette, LcdControl, LcdPosition, LcdStatus, MonochromePalette, ScanlinePosition,
};
...
impl Gpu {
    pub fn new(emu_mode: EmulationMode) -> Self {
        let position = LcdPosition::default();
        let scanline_pos = ScanlinePosition::from(&position);
    ...
    fn sprite_fetcher_tick(&mut self) {
        match self.sprite_fetcher.state {
            ...
            FetcherState::Push(1) => {
                ...
                // check if sprite needs some pixels to be discarded
                if sprite.x < 8 {
                    self.sprite_fifo.unaligned_objx = 8 - sprite.x;
                }
        ...
        match self.bg_fetcher.state {
            ...
            FetcherState::ReadTileNumber => {
                let (base, row, col) = if self.drawing_window {
                    ...
                    let row = self.position.ly.wrapping_add(self.scanline_pos.scroll_y) / 8;
                    let col = (self.scanline_pos.scroll_x / 8 + self.bg_fetcher.x) % 32;
                    (base, row, col)
                };
            ...
            FetcherState::ReadTileDataLow => {
                let row = if self.drawing_window {
                    ...
                    self.position.ly.wrapping_add(self.scanline_pos.scroll_y) % 8
                };
    ...
    fn is_win_enabled(&self) -> bool {
        self.lcdc.window_enabled(&self.emu_mode)
            && (self.scanline_pos.window_x < 167)
            && (self.scanline_pos.window_y < 144)
    }

    #[inline]
    fn is_win_pixel(&self) -> bool {
        self.scanline_pos.window_x <= (self.position.lx + 7) as u8
            && self.scanline_pos.window_y <= self.position.ly
    }
    ...
    fn change_mode(&mut self, mode: GpuMode) {
        ...
        if self.stat.mode == GpuMode::PixelTransfer {
            ...
            self.position.lx = 0;
            // self.position.slx = 0;
            self.position.slx = if self.oam_search.comparators.is_empty() {
                8
            } else {
                min!(self.oam_search.comparators[0], 8)
            };
            ...
            self.scanline_pos = ScanlinePosition::from(&self.position);

            self.fifo.unaligned_scx = self.scanline_pos.scroll_x % 8;

            if self.is_win_enabled()
                && self.scanline_pos.window_x < 7
                && self.scanline_pos.window_y <= self.position.ly
            {
                self.drawing_window = true;
                self.fifo.unaligned_winx = 7 - self.scanline_pos.window_x;
            } else {
